...
export class RegistroEspecialistaComponent implements OnInit {
  ...
  // EXTENDER LA LISTA CUANDO SEA NECESARIO
  especialidadesBase = [
  ...
  ngOnInit(): void {
    this.registroForm = this.fb.group({
      ...
      imagenPerfil:     this.fb.control<File | null>(null, Validators.required),
        ...
        captcha: this.fb.nonNullable.control(false, Validators.requiredTrue) // ← no-nullable


    });

    // Si el captcha está globalmente deshabilitado, liberá el form
    if (!this.captchaEnabled) {
      this.registroForm.controls.captcha.setValue(true);
      this.registroForm.controls.captcha.disable({ emitEvent: false });
    }
  ...
  onCaptcha(ev: boolean) {
    this.registroForm.controls.captcha.setValue(ev);
  }
  ...
  async onSubmit(): Promise<void> {
    ...
    try {
      ...
      especialidades = Array.from(new Set(especialidades.map(s => s?.trim())))
      ...
      await this.upsertPerfil({
        id: userId,
        rol: 'especialista',
        nombre: fv.nombre!,
        apellido: fv.apellido!,
        dni: fv.dni!,
        obra_social: null,                 // no aplica para especialista
        fecha_nacimiento: fv.fechaNacimiento!,
        email: fv.email!,
        avatar_url: avatarUrl,
        imagen2_url: null,                 // no usamos segunda imagen
        aprobado: false
        // especialidades                    // DESCOMENTAR SI LA TABLA TIENE ESTA COLUMNA
      });
...
//       captcha:          FormControl<boolean>;            // <===== CAPTCHITA 
...
//     //     captcha:          this.fb.control<boolean>(false, Validators.requiredTrue) // ←  captchita
...
//             otraEspecialidad: this.fb.control<string | null>(null),
//             imagenPerfil:     this.fb.control<File | null>(null, Validators.required),
//             captcha:          this.fb.control<boolean>(false, Validators.requiredTrue) // ← agregado
//           });
