...
// import { MatDialog, MatDialogModule } from '@angular/material/dialog';
// import { MatProgressSpinnerModule } from '@angular/material/progress-spinner';
// // 1. IMPORTAR MATSNACKBAR Y JSPDF
// import { MatSnackBar, MatSnackBarModule } from '@angular/material/snack-bar';
// import jsPDF from 'jspdf';
...
//     MatDialogModule,
//     MatProgressSpinnerModule,
//     MatSnackBarModule, // Asegurarse de importar el modulo
//     CapitalizarNombrePipe
//   ],
//   templateUrl: './usuarios-admin.component.html',
//   styleUrls: ['./usuarios-admin.component.scss'],
...
//         .from('usuarios')
//         .select('id, perfil, esta_aprobado, nombre, apellido, email, dni, obra_social, imagen_perfil_1, edad, fecha_registro, activo')
//         .order('apellido', { ascending: true });

//       if (error) throw error;

//       this.usuarios = (data || []).map((u: any): UsuarioAdmin => ({
//         id: u.id,
//         rol: u.perfil as Rol,
...
//           ? new Date(t.fecha_hora_inicio).toLocaleString('es-AR', { dateStyle: 'medium', timeStyle: 'short' })
//           : 'Fecha no disponible';
...
//       .from('turnos')
//       .select(`
//         id, fecha_hora_inicio, motivo, comentario,
...
//     return new Date(fechaISO).toLocaleDateString('es-AR', { year: 'numeric', month: 'long', day: 'numeric' });
//   }
...
//       const turnos = await this.obtenerTurnosUsuario(usuario);
//       if (!turnos.length) {
//         Swal.fire('Sin turnos', 'No encontramos turnos asociados.', 'info');
//         return;
//       }
//       const datosExcel = turnos.map(t => ({
//         'Turno ID': t.id,
//         Fecha: t.fecha_hora_inicio ? new Date(t.fecha_hora_inicio).toLocaleString('es-AR') : '',
...
//       const wb = XLSX.utils.book_new();
//       XLSX.utils.book_append_sheet(wb, ws, 'Turnos');
//       XLSX.writeFile(wb, `turnos_${usuario.apellido}_${new Date().getTime()}.xlsx`);
...
//       doc.setFontSize(12);
//       doc.setFont('helvetica', 'normal');
//       const rolTexto = usuario.rol === 'ESPECIALISTA' ? 'Especialista' : 'Usuario';
...
//       const labelContraparte = usuario.rol === 'ESPECIALISTA' ? 'PACIENTE' : 'PROFESIONAL';
...
//         const fechaTxt = t.fecha_hora_inicio ? new Date(t.fecha_hora_inicio).toLocaleDateString('es-AR') : '-';
//         const especTxt = t.especialidad?.nombre || 'Varios';
//         const userObj = usuario.rol === 'ESPECIALISTA' ? t.paciente : t.especialista;
//         const nombreContraparte = userObj ? `${userObj.apellido}, ${userObj.nombre}` : '-';
//         const estadoTxt = (t.estado?.codigo || 'PENDIENTE').toUpperCase();

//         doc.text(fechaTxt, colFecha, y);
//         doc.text(especTxt.substring(0, 20), colEspec, y);
