...
@Component({
  ...
  imports: [
    ...
    MatButtonModule,         // mat-button / mat-icon-button
    MatTooltipModule,        // matTooltip
    MatTableModule,          // mat-table, [dataSource]
...
export class MisTurnosEspecialistaComponent implements OnInit {
  dataSource = new MatTableDataSource<TurnoEspecialistaVM>([]); // <==== VM
  ...
  ngOnInit(): void {
    this.turnoService.getTurnosEspecialista$().subscribe({
      next: (ts) => {
        this.dataSource.data = ts; // ahora el tipo coincide
  ...
  cancelarTurno(turno: TurnoEspecialistaVM): void {
    const ref = this.dialog.open(this.confirmDialog, {
      data: { message: `Indica motivo de cancelación para el turno #${turno.id}` }
    });
    ref.afterClosed().subscribe(ok => {
      if (ok) {
        turno.estado = 'cancelado' as EstadoTurno;
  ...
  finalizarTurno(turno: TurnoEspecialistaVM): void {
    const ref = this.dialog.open(this.confirmDialog, {
  ...
  verResena(turno: TurnoEspecialistaVM): void {
...
//   templateUrl: './mis-turnos-especialista.component.html',
//   styleUrls: ['./mis-turnos-especialista.component.scss'],
//   imports: [MatCardModule, MatFormFieldModule, MatIconModule, MatDialogModule,
//   // Angular
//   CommonModule,            // *ngIf, *ngFor
//   TitleCasePipe,           // | titlecase
//   // Angular Material
...
// import { Router } from '@angular/router';
// import { TurnoService } from '../../../services/turno.service';
// import { Turno, TurnoEspecialista } from '../../../../models/interfaces';
