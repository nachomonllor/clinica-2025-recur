...
//   exportarPdf(): void {
//     const turnosParaExportar = this.turnos; // Usamos los turnos filtrados actuales
...
//     // Intentamos obtener el nombre del paciente del servicio de auth o usamos un genérico
//     // Ajusta esto según cómo guardes el nombre en tu auth service
//     const nombrePaciente = this.supa.user?.user_metadata?.['nombre'] 
//                            ? `${this.supa.user.user_metadata['nombre']} ${this.supa.user.user_metadata['apellido']}`
//                            : 'Paciente';

//     // 1. EL SVG DEL LOGO (Mismo que admin)
//     const svgLogo = `
//   <svg width="600" height="200" viewBox="0 0 600 200" xmlns="http://www.w3.org/2000/svg">
//     <defs>
//       <linearGradient id="gradBlue" x1="0%" y1="0%" x2="100%" y2="100%">
...
//         doc.setFont('helvetica', 'bold');
//         doc.setFontSize(16);
//         doc.text('Mis Turnos / Historia Clínica', pageWidth - marginX, 18, { align: 'right' });

//         doc.setFont('helvetica', 'normal');
//         doc.setFontSize(10);
//         doc.text(`Paciente: ${nombrePaciente}`, pageWidth - marginX, 24, { align: 'right' });
...
//       // --- CONTENIDO ADAPTADO PARA EL PACIENTE ---
//       const contentWidth = pageWidth - marginX * 2;

//       turnosParaExportar.forEach((t: TurnoVM, index: number) => {
//         const paragraphs: { lines: string[]; bold?: boolean }[] = [];
//         const anchoTexto = contentWidth - cardPadding * 2;
...
//         // 1. Cabecera de la tarjeta (Fecha y Estado)
//         const fechaStr = t.fecha ? t.fecha.toLocaleDateString('es-AR') : 'Sin fecha';
//         addParagraph(`Turno #${index + 1} · ${fechaStr} · ${t.hora} hs · (${t.estado?.toUpperCase()})`, { bold: true });

//         // 2. Datos del profesional
//         addParagraph(`Especialidad: ${t.especialidad}`);
//         addParagraph(`Especialista: ${t.especialista}`);

//         // 3. Historia Clínica / Reseña
//         // En tu TurnoVM, la historia a veces viene como 'historiaClinica' (objeto) o 'historiaBusqueda' (string)
//         // Intentamos castear para ver si hay datos detallados
//         const dataHistoria: any = (t as any).historiaClinica;

//         if (dataHistoria && typeof dataHistoria === 'object') {
//           // CASO A: Tenemos objeto con altura, peso, etc.
//           let detalles = '';
//           if (dataHistoria.altura) detalles += `Altura: ${dataHistoria.altura} cm | `;
//           if (dataHistoria.peso) detalles += `Peso: ${dataHistoria.peso} kg | `;
//           if (dataHistoria.temperatura) detalles += `Temp.: ${dataHistoria.temperatura} °C | `;
//           if (dataHistoria.presion) detalles += `Presión: ${dataHistoria.presion}`;
          
//           if (detalles) addParagraph(detalles);

//           // Datos dinámicos si existen dentro del objeto
//           if (dataHistoria.datos_dinamicos && Array.isArray(dataHistoria.datos_dinamicos)) {
//              addParagraph('Datos dinámicos:', { bold: true });
//              dataHistoria.datos_dinamicos.forEach((d: any) => addParagraph(`• ${d.clave}: ${d.valor}`));
//           }

//         } else if (t.historiaBusqueda) {
//           // CASO B: Solo tenemos el string plano de búsqueda
//           addParagraph(`Detalles clínicos: ${t.historiaBusqueda}`);
//         } else {
//           // CASO C: Sin datos
//            addParagraph('Sin datos clínicos detallados disponibles.');
//         }

//         // 4. Reseña del especialista (si existe)
//         if (t.resena) {
//           addParagraph(`Reseña/Comentario: ${t.resena}`);
//         }
...
//         // DIBUJAR TARJETA
//         doc.setFillColor(248, 250, 252); // Un gris muy clarito para el paciente (diferente al amarillo del admin)
//         doc.setDrawColor(59, 130, 246); // Borde azulado estilo "clínica"
//         doc.setLineWidth(0.4);
//         doc.roundedRect(marginX, y, contentWidth, cardHeight, 3, 3, 'FD');
...
//       const nombreArchivo = `mis_turnos_${new Date().getTime()}.pdf`;
//       doc.save(nombreArchivo);
//     };

//     // 3. CARGA Y CONVERSIÓN DE IMAGEN (Igual que admin)
//     const img = new Image();
//     img.src = svgBase64;
