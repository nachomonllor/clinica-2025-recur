...
export class TurnosService {
  ...
  private async obtenerTurnosPacienteVM(): Promise<TurnoVM[]> {
    ...
    return turnos.map((t): TurnoVM => {
      ...
      // AQUÍ OBTENEMOS EL CÓDIGO CORRECTO DEL MAPA
      const codigoEstado = estadosMap.get(t.estado_turno_id) ?? 'PENDIENTE';

      // --- CORRECCIÓN ---
      // Usamos mapEstadoCodigoToUI si lo deseas, o simplemente codigoEstado
      // El problema es que antes asignabas "t.estado_turno?.codigo" y eso no existía en el SELECT inicial.
      ...
      return {
        ...
        // CORREGIDO: Usamos la variable codigoEstado que obtuvimos del Map
        estado: codigoEstado.toUpperCase(),
      };
    });
  }
